...
export default function ChecklistPage() {
  ...
  function submitNewItem() {
    ...
    const targetCat = newItemCategory || (cats[0]?.name ?? 'General')
    
    setCategories(prev => {
      const tripCats = [...(prev[tripId] ?? CHECKLISTS['1'])]
      const catIdx = tripCats.findIndex(c => c.name === targetCat)
      
      if (catIdx >= 0) {
        tripCats[catIdx] = {
          ...tripCats[catIdx],
          items: [...tripCats[catIdx].items, { id: `custom-${Date.now()}`, label: newItemLabel, checked: false }]
        }
      }
      return { ...prev, [tripId]: tripCats }
    })
  ...
  return (
    <div className="p-6 md:p-8 space-y-6 max-w-3xl mx-auto">
      ...
      {isAdding && (
        <div className="flex items-center gap-3 border border-border rounded-xl p-4 bg-muted/30 shadow-sm animate-in fade-in slide-in-from-bottom-2">
          <Input 
            placeholder="What do you need to pack?" 
            value={newItemLabel} 
            onChange={e => setNewItemLabel(e.target.value)} 
            onKeyDown={e => e.key === 'Enter' && submitNewItem()}
            className="h-10 text-sm border-border bg-background flex-1" 
            autoFocus
          />
          <select 
          ...
          <button onClick={submitNewItem} className={cn(buttonVariants({ size: 'sm' }), "h-10 px-4")}>Add</button>
      ...
      <div className="flex gap-3 pt-2">
        <button onClick={() => setIsAdding(true)} className={cn(buttonVariants({ variant: 'outline' }), 'flex-1 border-border text-sm gap-1.5 h-10')}>
          + Add item
        </button>
